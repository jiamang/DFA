...
from model import DFA_partseg,PointNet
import numpy as np
...
def train(args, io):
    ...
    if args.model == 'dfa':
        model = DFA_partseg(args, seg_num_all).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        train_pred_seg = []
        train_label_seg = []
        for i,(data, label, seg) in tqdm(enumerate(train_loader, 0),total=len(train_loader),smoothing=0.9):
        ...
        test_label_seg = []
        with torch.no_grad():
            for i,(data,label, seg) in tqdm(enumerate(test_loader, 0),total=len(test_loader),smoothing=0.9):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='dfa', metavar='N',choices=['dfa','pointnet'],help='Model to use, [dfa]')
