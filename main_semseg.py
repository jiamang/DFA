...
from model import DFA_semseg, PointNet
import numpy as np
...
def visualization(visu, visu_format, test_choice, data, seg, pred, visual_file_index, semseg_colors):
    ...
    for i in range(0, data.shape[0]):
        ...
        RGB_gt = [] 
        skip = False
        with open("/home/data/lzh/indoor3d_sem_seg_hdf5_data_test/room_filelist.txt") as f:
            files = f.readlines()
        ...
        if skip:
            ...
            data = np.loadtxt('/home/data/lzh/indoor3d_sem_seg_hdf5_data_test/raw_data3d/Area_'+test_area+'/'+roomname+'('+str(visual_file_index)+').txt')
            # data = data[:, :3, :]
            ...
            for j in range(0, data.shape[0]):
                #print("j: ", j)
                #print(int(pred[i][j]))
                #print(semseg_colors[int(pred[i][j])])
...
def train(args, io):
    ...
    # root = '/home/lzh/Data/S3DIS/Stanford3dDataset_v1.2_Aligned_Version/stanford_indoor3d'
    # NUM_POINT = args.num_points
    # BATCH_SIZE = args.batch_size
    # TRAIN_DATASET = S3DISFull(split='train', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    # TEST_DATASET = S3DISFull(split='test', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    ...
    # train_data, train_label, test_data, test_label = recognize_all_data(test_area = 6)

    # dataset = S3DISDataLoader(train_data,train_label)
    # dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=True)
    # test_dataset = S3DISDataLoader(test_data,test_label)
    # testdataloader = torch.utils.data.DataLoader(test_dataset, batch_size=8, shuffle=True)

    device = torch.device("cuda" if args.cuda else "cpu")

    #Try to load models
    if args.model == 'dfa':
        model = DFA_semseg(args).to(device)
        #model = GACNet(13, args).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i,(data, seg) in tqdm(enumerate(train_loader, 0),total=len(train_loader),smoothing=0.9):
            data, seg = data.to(device), seg.to(device)     #data[B*4096*9]
            data = data.permute(0, 2, 1)
            batch_size = data.size()[0]
            opt.zero_grad()
            seg_pred = model(data)
            #seg_pred = model(data[:,:3,:],data[:,3:,:])
            seg_pred = seg_pred.permute(0, 2, 1).contiguous()
            loss = criterion(seg_pred.view(-1, 13), seg.view(-1,1).squeeze(),False)
            loss.backward()
            opt.step()
        ...
        with torch.no_grad():
            for i,(data, seg) in tqdm(enumerate(test_loader, 0),total=len(test_loader),smoothing=0.9):
...
def test(args, io):
    ...
    for test_area in range(1,7):
        ...
        if os.path.exists("/home/data/lzh/indoor3d_sem_seg_hdf5_data_test/room_filelist.txt"):
            with open("/home/data/lzh/indoor3d_sem_seg_hdf5_data_test/room_filelist.txt") as f:
                for line in f:
                    if (line[5]) == test_area:
                        break
        ...
        if (args.test_area == 'all') or (test_area == args.test_area):
            ...
            for data, seg in test_loader:
                ...
                data = data.permute(0, 2, 1)
                batch_size = data.size()[0]
                seg_pred = model(data)          #B*13*N
                #seg_pred = model(data[:,:3,:],data[:,3:,:])
                seg_pred = seg_pred.permute(0, 2, 1).contiguous()
                pred = seg_pred.max(dim=2)[1]       #[1]表示返回最大值的每个索引
                seg_np = seg.cpu().numpy()
                pred_np = pred.detach().cpu().numpy()
                test_true_cls.append(seg_np.reshape(-1))
                test_pred_cls.append(pred_np.reshape(-1))
            ...
            outstr = 'Test :: test area: %s, test acc: %.6f, test avg acc: %.6f, test iou: %.6f \n' % (test_area,
                                                                                                    ...
                                                                                                    avg_per_class_acc,
                                                                                                    np.mean(test_ious))
            print("class".ljust(10), "class_acc".ljust(10), "class_iou".ljust(10))
            for i in range(len(labels)):
                print(labels[i].ljust(10), str(round(per_class[i], 4)).ljust(10), str(round(test_ious[i], 4)).ljust(10))
    ...
    if args.test_area == 'all':
        ...
        avg_per_class_acc ,per_class= metrics.balanced_accuracy_score(all_true_cls, all_pred_cls)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='dfa', metavar='N',choices=['dfa','PointNet','GAC'],help='Model to use, [dfa]')
    parser.add_argument('--dataset', type=str, default='S3DIS', metavar='N',choices=['S3DIS'])
    parser.add_argument('--test_area', type=str, default=None, metavar='N',choices=['1', '2', '3', '4', '5', '6', 'all'])
    ...
    parser.add_argument('--visu_format', type=str, default='ply',help='file format of visualization')
